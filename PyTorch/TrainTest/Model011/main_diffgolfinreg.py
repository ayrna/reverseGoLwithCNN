...
ex = Experiment('DiffGoLFinReg Model')
...
def main(seed, delta, shape, parent_folder, model_names, remove_model_data, remove_test, remove_all,
    ...
    # loss_fn = tools.get_custom_loss('FocalBCEFuzz', lambda_init=lambda_init, lambda_phys=lambda_phys,lambda_bin=lambda_bin, gamma=gamma, alpha=alpha,
    ...
    model = tools.training_loop(shape, delta, seed, model_dirs, device,
                  model, num_outputs, loss_fn, optimizer, epochs, batch_size, epochs2hold,
                  tolerance_dts, tolerance_lr, factor, warmup_epochs, 
                  schedule_lambda_phys, schedule_lambda_bin, schedule_lambda_dice,
                  val_split, states2generate, margin, min_density, max_density, generator_batch_size)
