...
def config():
    ...
    delta = 1
    shape = (15,15)
    path_datos = './Resultados/Resultados_Clasico'
    epochs = 6
    hold = True
    epochs2hold = 2
    dict_hps = {
...
# ---------------- Paso 2.0: Inicialización modelo y creación del validation dataset ---------------- #
    model = builder.modelo_clasico(dict_hps, shape)
...
# ---------------- Paso 2.2: Transformación del dataset para TF  ---------------- #
        ...
        Y_train = iniciales_train.reshape((-1, shape[0], shape[1], 1)) # (k, M,M, 1)
        ...
        Y_val = iniciales_val.reshape((-1, shape[0], shape[1], 1)) # (k, M,M, 1)
...
# ---------------- Paso 2.4: Callbacks ---------------- #
    ...
    # Generación de datos para test:
    dynamic.board_generator(seed = seed, 
                            delta=delta, 
                            shape=shape, 
                            dir_npz=dir_boards_test, 
                            name=f'test_{seed}', TrainVal=False, n_objetivo= 15_000
                            )  # val_split=0.2, num_tableros = 120k, densidad_min= 0.05, densidad_max = 0.95, batch_size=5_000)
