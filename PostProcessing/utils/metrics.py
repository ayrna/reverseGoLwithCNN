...
def computeCM(y_pred, y_true, metrics2compute):
    """Compute confusion matrix metrics across multiple seeds.
    
    Computes specified metrics for binary classification predictions across
    multiple seeds. It does not requires for thresholding.
    
    Args:
        y_pred (list): List of prediction arrays, one per seed with shape 
            (n_boards, n_cells).
        y_true (list): List of ground truth arrays, one per seed with shape 
            (n_boards, n_cells).
        threshold (float): Classification threshold for binary predictions.
        metrics2compute (list): List of metric names to compute (e.g., 
            ['accuracy', 'precision', 'recall']).
    
    Returns:
        tuple: A tuple containing:
            - mean_values (np.ndarray): Mean metric values across seeds, 
              shape (n_metrics,).
            - stds (np.ndarray): Standard deviations of metrics across seeds, 
              shape (n_metrics,).
    """
    # Loop
...
def computeCM_bxb(y_pred, y_true, thresholds, metrics2compute):
    """
    Compute specified binary metrics per-board using provided thresholds and
    return mean and standard deviation across seeds.

    This function iterates over seeds. For each seed it converts predictions
    and ground-truth arrays to tensors, applies a per-board threshold list to
    compute metrics for each board, averages the metric values across boards
    for the seed, and finally computes the mean and standard deviation of
    those per-seed averages for each requested metric.

    Args:
        y_pred: Iterable of prediction arrays per seed. Each element should be
            convertible to a torch tensor of shape (n_boards, n_cells) or
            (n_boards, H, W) depending on representation.
        y_true: Iterable of ground-truth arrays per seed. Each element should
            be convertible to a torch tensor with same first-dimension
            (n_boards) as y_pred.
        thresholds: Iterable of iterables. For each seed, provides a list of
            thresholds, one per board (length must equal n_boards). Thresholds
            are cast to float before use.
        metrics2compute: Sequence of metric names (strings) to compute. Metrics
            are resolved via tools.get_metric with task='binary'.

    Returns:
        mean_values: numpy.ndarray of shape (len(metrics2compute),) containing
            the mean of per-seed averaged metric values for each metric.
        stds: numpy.ndarray of shape (len(metrics2compute),) containing the
            standard deviation of the per-seed averaged metric values.

    Raises:
        AssertionError: if the number of thresholds for a seed does not match
            the number of boards in the corresponding prediction tensor.
    """
    ...
    for pred, gtruth, ths in zip(y_pred, y_true, thresholds):
        ...
        # Convert to tensor -->  shape (n_seeds, n_boards, n_cells)
        pred = torch.tensor(pred, dtype=torch.float32)
        gtruth = torch.tensor(gtruth, dtype=torch.uint8)
...
def otsu_per_board(y_pred, shape=(15, 15)):
    """Applies Otsu's thresholding method to binarize predicted boards.
    
    Computes optimal thresholds for each board using Otsu's method
    
    Args:
        y_pred: Predicted values to binarize. Can be a 1D or multi-dimensional array.
        shape: Target shape for reshaping boards. Defaults to (15, 15).
    
    Returns:
        - thresholds: 1D array of optimal threshold values for each board (normalized to [0, 1]).
...
def gauss_per_board(y_pred, shape=(15, 15), block_size=3, C=0):
    """Binarize predicted boards using adaptive Gaussian thresholding per board.

    This function converts predicted continuous-valued boards to binary boards by
    applying OpenCV's adaptive Gaussian thresholding to each board individually.

    Args:
        y_pred: Array-like of predicted values. Can be a flat or batched array which
            will be reshaped to (-1, *shape).
        shape: Tuple[int, int], target spatial shape for each board. Defaults to (15, 15).
        block_size: Odd integer specifying the size of the neighbourhood used to
            calculate the adaptive threshold. Defaults to 3.
        C: Constant subtracted from the mean or weighted sum in the adaptive thresholding.
            Defaults to 0.

    Returns:
        np.ndarray: Array of binarized boards with the same batch dimension as the
        reshaped input and dtype uint8. Values are 0 or 1.
    """
    ...
    for i, board in enumerate(boards):
        ...
        board_bin = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, block_size, C)
