...
def training_loop_regressor(model:nn.Module, optimizer:torch.optim, loss_fn:nn.Module, dict2data:dict[str:Path], 
                            dict2outputs:dict[str:Path], shape:tuple, epochs:int, batch_size:int, device:torch.device):
    ...
    df_train = pd.read_csv(dict2data['train'], sep=',')
    df_val = pd.read_csv(dict2data['val'], sep=',')

    # Data:
    x_train = torch.tensor(df_train[start_cols].values, dtype=torch.float32).reshape(-1, 1, H,W)
    x_val = torch.tensor(df_val[start_cols].values, dtype=torch.float32).reshape(-1, 1, H,W)
    y_train = torch.tensor(df_train['threshold'].values, dtype=torch.float32).reshape(-1, 1)
    y_val = torch.tensor(df_val['threshold'].values, dtype=torch.float32).reshape(-1,1)
    ...
    # Training Loop:
    best_val_loss = float('inf')
    file_model = dict2outputs['model']
    file_train_history = dict2outputs['history']
    global_train_history = []
    for epoch in range(epochs):
        ...
        step_losses = 0
        model.train()
        for heatmap_train, threshold_train in train_loader:
            # Reset optimizer:
            optimizer.zero_grad()
            # Send data to device:
            heatmap_train = heatmap_train.to(device)
            threshold_train = threshold_train.to(device)
            # Forward:
            threshold_pred = model(heatmap_train)
            step_loss = loss_fn(threshold_pred, threshold_train)
            # Backprop:
            step_loss.backward()
            optimizer.step()
            step_losses += step_loss.item()
        ...
        model.eval()
        with torch.no_grad():
            for heatmap_val, threshold_val in val_loader:
                # Send data to device:
                heatmap_val = heatmap_val.to(device)
                threshold_val = threshold_val.to(device)
                # Prediction:
                threshold_pred = model(heatmap_val)
                step_val_losses += loss_fn(threshold_pred, threshold_val).item()
...
def test_loop_regressor(model:nn.Module, dict2data:dict[str:Path], metrics2compute:list[str], shape:tuple[int,int], batch_size:int, device:torch.device):
    ...
    # Test dataset
    df_test = pd.read_csv(dict2data['test'], sep=',')
    
    # Data:
    x_test = torch.tensor(df_test[start_cols].values, dtype=torch.float32).reshape(-1, 1, H,W)
    y_test = torch.tensor(df_test['threshold'].values, dtype=torch.float32).reshape(-1, 1)
    
    test_loader = DataLoader(TensorDataset(x_test, y_test), batch_size=batch_size, shuffle=False)
    
    # Metrics to compute
    metrics = {metric: get_metric(metric).to(device) for metric in metrics2compute}

    # Evaluation:
    predictions = []
    model.eval()
    with torch.no_grad():
        for heatmap, threshold in test_loader:
            # Send to device:
            heatmap = heatmap.to(device)
            ths = threshold.to(device)
            
            # Predict:
            ths_pred = model(heatmap)

            # Update metrics:
            for _ , metric_fn in metrics.items():
                metric_fn.update(ths_pred, ths)

            # Save predictions:
            predictions.append(torch.cat([ths_pred, ths], dim=1).cpu())
     # Compute and save test results:
    test_results = {}
    
    test_results = {name: m.compute().item() for name, m in metrics.items()}
    for m in metrics.values():
        m.reset()

    file_test_results = dict2data['results']
    file_predictions = dict2data['predictions']
    pd.DataFrame([test_results]).to_csv(file_test_results, index=False)
    predictions = torch.cat(predictions, dim=0).numpy()
    pd.DataFrame(predictions, columns=['threshold_pred', 'threshold_true']).to_csv(file_predictions, index=False)
