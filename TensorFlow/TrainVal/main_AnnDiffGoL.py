...
ex = Experiment('Train_DynamicReg')
...
def config():
    ...
    delta = 1
    shape = (15,15)
    path_datos = './Resultados/Resultados_DynamicReg'
    epochs = 6
    hold = True
    epochs2hold = 1000
    dict_hps = {
...
# ---------------- Paso 1.4: Limpieza de directorios ---------------- #
    ...
    files2remove = [file_test_seed, file_history, file_model, file_tensorboard]
    ...
    # Creamos directorios
    dirs2create = [dir_boards_train_seed, dir_boards_val_seed, dir_boards_test, dir_history, dir_modelos, dir_tensorboard]
...
# ---------------- Paso 2.0: Inicialización modelo y creación del validation dataset ---------------- #
    model = builder.modelo_complejo_dinamico(dict_hps, shape, delta)

    # Variables para control manual de los callbacks
    best_val_loss = float('inf')
    best_endgame_val_loss = float('inf')
    history_global = [] # Lista para acumular diccionarios
    patience_cnt = 0    # Contador para LR manual 
    patience_dts = 0
    new_lambda_phys = 0
    new_lambda_bin = 0
    schedule_lambda_phys = {250: 0.25, 333: 0.50, 416: 0.75, 499: 1.00}
    schedule_lambda_bin = {600: 0.5, 650: 0.75, 700: 1.0, 750:1.25, 799:1.5}
...
# ---------------- Paso 2.1: Creación y carga de dataset dinámico (k, H*W) ---------------- #
        ...
        # Check si hay que hacer cambio por época:
        ...
        # Check si hay que ajustar el valor de los lambdas:
        if epoch in schedule_lambda_phys:
            new_lambda_phys = schedule_lambda_phys[epoch]
            model.lambda_phys.assign(schedule_lambda_phys[epoch])
            print(f"Update lambda_phys: {schedule_lambda_phys[epoch]}")
        
        if epoch in schedule_lambda_bin:
            new_lambda_bin = schedule_lambda_bin[epoch]
            model.lambda_bin.assign(schedule_lambda_bin[epoch])
            print(f"Update lambda_bin: {schedule_lambda_bin[epoch]}")
        
        # Tras checkeo de cambio de dataset:
        if (hold==True):
...
# ---------------- Paso 2.4: Callbacks ---------------- #
        ...
        # CheckPoint
        if epoch >= 800:
            if current_val_loss < best_endgame_val_loss:
                best_endgame_val_loss = current_val_loss
                print('Model Checkpoint. Mejora detectada')
        ...
        # Para los contadores de LR y de dataset change:
        if current_val_loss < best_val_loss:
            print(f"Mejora detectada ({best_val_loss:.4f} -> {current_val_loss:.4f}). Reseteando contadores.")
        ...
        metrics['Lambda_phys'] = new_lambda_phys
        metrics['Lambda_bin'] = new_lambda_bin
